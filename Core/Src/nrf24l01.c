...
NRF_RESULT nrf_init(nrf24l01* dev, nrf24l01_config* config) {
    ...
    nrf_set_rx_payload_width(dev, dev->config.payload_length,NRF_P0);
    nrf_set_rx_payload_width(dev, dev->config.payload_length,NRF_P1);
    //nrf_set_rx_payload_width_p1(dev, dev->config.payload_length);

    //nrf_set_rx_address_p1(dev, dev->config.rx_address);
    nrf_set_rx_address(dev, dev->config.tx_address,NRF_P1);
    nrf_set_rx_address(dev, dev->config.tx_address,NRF_P0);
...
NRF_RESULT nrf_set_rx_address(nrf24l01* dev, const uint8_t* address, NRF_PIPES pipe) {
    uint8_t rx[5];
    if (nrf_send_command(dev, NRF_CMD_W_REGISTER | (NRF_RX_ADDR_P0 + pipe), address, rx,
                         5) != NRF_OK) {
        return NRF_ERROR;
    }
...
NRF_RESULT nrf_set_rx_payload_width(nrf24l01* dev, uint8_t width, NRF_PIPES pipe) {
    width &= 0x3F;
    if (nrf_write_register(dev, NRF_RX_PW_P0 + pipe, &width) != NRF_OK) {
